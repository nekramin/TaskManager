...
namespace TaskManagerDesktop
{
    public static class ModelHelper
    {
        public static ObservableCollection<TaskItem> CreateSampleTasks()
        {
            ...
            for (int i = 1; i <= 10; i++)
            {
                var task = new TaskItem
                {
                    ...
                    Description = $"Это тестовое описание для задачи #{i}. Задача создана для демонстрации работы приложения.",
                    DueDate = DateTime.Now.AddDays(rnd.Next(-2, 7)),
                    IsCompleted = rnd.Next(0, 3) == 0,
        ...
        {
            if (task == null) return (false, "Задача не может быть null");
            if (string.IsNullOrWhiteSpace(task.Title)) return (false, "Название задачи обязательно");
            if (task.Title.Length > 200) return (false, "Название слишком длинное (максимум 200 символов)");
            if (task.Description?.Length > 1000) return (false, "Описание слишком длинное (максимум 1000 символов)");
            if (task.DueDate < DateTime.Now.AddMinutes(-5)) return (false, "Дата выполнения не может быть в прошлом");
            if (task.Priority == null) return (false, "Приоритет не указан");
            if (task.Category == null) return (false, "Категория не указана");
